...
├── config/             # ClockConfig, RestClientConfig, SwaggerConfig
├── currency/           # Currency, RatedCurrency
├── exchangerate
...
│   ├── entity/         # ExchangeRateHistoryEntity
...
│   └── service/        # ExchangeRateService (인터페이스), InMemoryExchangeRateService
└── order
    ...
    └── service/        # OrderService, OrderTransactionService
```
...
erDiagram
    exchange_order_request {
        BIGINT id PK
        DECIMAL forex_amount
        VARCHAR from_currency
        VARCHAR to_currency
        VARCHAR status "RECEIVED / SUCCESS / FAILED"
    ...
    exchange_order {
        BIGINT id PK
        BIGINT request_id FK "UNIQUE"
        VARCHAR direction "BUY / SELL"
        VARCHAR currency
        DECIMAL forex_amount
        DECIMAL krw_amount
        DECIMAL trade_rate
        DATETIME rate_date_time
        DATETIME created_at
    }
    exchange_rate_history {
        BIGINT id PK
    ...
    exchange_order_request ||--o| exchange_order : "request_id (UNIQUE)"
```
...
```mermaid
sequenceDiagram
    participant Scheduler as InitialCollector / Scheduler
    participant Collector as ProviderFallbackExchangeRateCollector
    participant Provider as Provider (1→2→3 순서 시도)
    participant PS as ExchangeRatePersistenceService
    participant Cache as InMemoryExchangeRateService
    participant DB

    Scheduler->>Collector: collectAll()

    loop USD · JPY · CNY · EUR 각 통화
        loop Provider 순서대로 (fallback)
            Collector->>Provider: fetchRate(currency, KRW)
            Provider-->>Collector: ProviderRate
            Note over PS,DB: REQUIRES_NEW TX
            Collector->>PS: saveIfValid(providerRate)
            PS->>DB: 직전 환율 조회 (sanity check)
            alt 유효 (sanity check 통과)
                PS->>DB: INSERT exchange_rate_history
                PS-->>Collector: Optional[SavedExchangeRate]
                Collector->>Cache: update(currency, response)
                Note over Collector: 다음 통화로
            else 무효 또는 sanity check 초과
                PS-->>Collector: Optional.empty()
                Note over Collector: 다음 Provider로 fallback
            end
        end
    end
```
...
트랜잭션 경계가 필요한 저장/실패 처리를 `OrderTransactionService`로 분리하고 `@Transactional(propagation = REQUIRES_NEW)`를 적용했습니다.
별도 빈으로 분리하면 Spring 프록시가 정상 동작하여 각 호출마다 새 트랜잭션이 열립니다.

```mermaid
sequenceDiagram
    participant Client
    participant OS as OrderService
    participant OTS as OrderTransactionService
    participant Cache as InMemoryExchangeRateService
    participant DB

    Client->>OS: POST /order

    Note over OTS,DB: TX-1 (REQUIRES_NEW)
    OS->>OTS: saveRequest()
    OTS->>DB: INSERT exchange_order_request (RECEIVED)
    OTS-->>OS: ExchangeOrderRequestEntity

    OS->>Cache: getLatestRate(from, to)
    Cache-->>OS: ExchangeRateResponse

    alt 환율 만료 또는 유효성 오류
        Note over OTS,DB: TX-3 (REQUIRES_NEW)
        OS->>OTS: markFailed(errorCode)
        OTS->>DB: UPDATE exchange_order_request (FAILED)
        OS-->>Client: 4xx / 503
    else 정상 처리
        Note over OTS,DB: TX-2 (REQUIRES_NEW)
        OS->>OTS: saveOrder(order, request)
        OTS->>DB: INSERT exchange_order
        OTS->>DB: UPDATE exchange_order_request (SUCCESS)
        OS-->>Client: 200 OrderResponse
    end
```

### 환율 조회: 인메모리 캐시

수집 성공 시 `ExchangeRatePersistenceService`가 DB에 저장한 뒤, `InMemoryExchangeRateService`의 캐시를 즉시 갱신합니다.
주문 처리와 환율 조회 API는 DB 쿼리 없이 캐시(`ConcurrentHashMap`)에서 읽으므로 조회 지연과 DB 부하를 줄입니다.
캐시는 재시작 시 초기 수집(`ExchangeRateInitialCollector`) 이후 다시 채워집니다.

### 환율 신선도: 주문 시점 검증

환율 수집은 배경 작업이라 실시간성 요구가 없고, 데이터 품질은 Sanity Check가 담당합니다.
그러나 수집 실패가 연속되면 캐시에 오래된 환율만 남을 수 있습니다.
이때 오래된 환율로 주문이 체결되는 것을 막아야 하므로, 신선도는 "사용 시점"에 판단합니다.

캐시에서 조회한 환율의 수집 시각이 기준 시간(기본 5분)을 초과하면 `503 RATE_STALE`을 반환합니다.
...
| InMemoryExchangeRateService | `@ExtendWith(MockitoExtension.class)` | 전체/단일 환율 조회, 캐시 갱신, 환율 없을 시 예외 |
...
| ExchangeRateInitialCollector / ExchangeRateCollectionScheduler | `@ExtendWith(MockitoExtension.class)` | 초기 수집 및 스케줄 수집 호출 검증 |
