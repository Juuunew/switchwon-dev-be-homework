...
`ExchangeRateInitialCollector`가 애플리케이션 시작 시 최초 수집을 수행합니다.
`ProviderFallbackExchangeRateCollector`는 통화별로 독립적인 fallback 수집을 담당합니다.
...
- 외부 API 호출은 트랜잭션 밖에서 수행하고, 직전 환율 조회 및 저장만 트랜잭션으로 처리
